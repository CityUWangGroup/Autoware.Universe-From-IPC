...
set(_exported_dependencies "autoware_auto_perception_msgs;autoware_auto_planning_msgs;diagnostic_msgs;motion_utils;nav_msgs;pcl_conversions;pcl_ros;rclcpp;rclcpp_components;sensor_msgs;signal_processing;std_msgs;tf2;tf2_eigen;tf2_geometry_msgs;tf2_ros;tier4_autoware_utils;tier4_debug_msgs;tier4_planning_msgs;vehicle_info_util;visualization_msgs")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to obstacle_stop_planner_DEFINITIONS, obstacle_stop_planner_INCLUDE_DIRS,
# obstacle_stop_planner_LIBRARIES, and obstacle_stop_planner_LINK_FLAGS.
# Additionally collect the direct dependency names in
# obstacle_stop_planner_DEPENDENCIES as well as the recursive dependency names
# in obstacle_stop_planner_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(obstacle_stop_planner_DEPENDENCIES ${_exported_dependencies})
  set(obstacle_stop_planner_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(obstacle_stop_planner_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(obstacle_stop_planner_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(obstacle_stop_planner_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(obstacle_stop_planner_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(obstacle_stop_planner_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND obstacle_stop_planner_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
