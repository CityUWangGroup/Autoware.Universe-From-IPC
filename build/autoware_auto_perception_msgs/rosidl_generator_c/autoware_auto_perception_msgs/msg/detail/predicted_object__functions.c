  ...
  autoware_auto_perception_msgs__msg__PredictedObject__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_auto_perception_msgs__msg__PredictedObject * data =
      (autoware_auto_perception_msgs__msg__PredictedObject *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_auto_perception_msgs__msg__PredictedObject__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_auto_perception_msgs__msg__PredictedObject__fini(&data[i]);
        }
        free(data);
        return false;
      }
