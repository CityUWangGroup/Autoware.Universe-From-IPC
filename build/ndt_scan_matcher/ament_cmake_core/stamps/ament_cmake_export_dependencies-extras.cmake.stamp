...
set(_exported_dependencies "diagnostic_msgs;fmt;geometry_msgs;nav_msgs;ndt_omp;pcl_conversions;rclcpp;sensor_msgs;std_msgs;std_srvs;tf2;tf2_eigen;tf2_geometry_msgs;tf2_ros;tf2_sensor_msgs;tier4_autoware_utils;tier4_debug_msgs;tier4_localization_msgs;visualization_msgs")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to ndt_scan_matcher_DEFINITIONS, ndt_scan_matcher_INCLUDE_DIRS,
# ndt_scan_matcher_LIBRARIES, and ndt_scan_matcher_LINK_FLAGS.
# Additionally collect the direct dependency names in
# ndt_scan_matcher_DEPENDENCIES as well as the recursive dependency names
# in ndt_scan_matcher_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(ndt_scan_matcher_DEPENDENCIES ${_exported_dependencies})
  set(ndt_scan_matcher_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(ndt_scan_matcher_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(ndt_scan_matcher_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(ndt_scan_matcher_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(ndt_scan_matcher_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(ndt_scan_matcher_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND ndt_scan_matcher_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
