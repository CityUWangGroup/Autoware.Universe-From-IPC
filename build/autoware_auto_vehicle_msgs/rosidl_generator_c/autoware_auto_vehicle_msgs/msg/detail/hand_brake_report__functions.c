...
autoware_auto_vehicle_msgs__msg__HandBrakeReport__Sequence__are_equal(const autoware_auto_vehicle_msgs__msg__HandBrakeReport__Sequence * lhs, const autoware_auto_vehicle_msgs__msg__HandBrakeReport__Sequence * rhs)
{
  ...
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!autoware_auto_vehicle_msgs__msg__HandBrakeReport__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
  ...
  autoware_auto_vehicle_msgs__msg__HandBrakeReport__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(autoware_auto_vehicle_msgs__msg__HandBrakeReport);
    autoware_auto_vehicle_msgs__msg__HandBrakeReport * data =
      (autoware_auto_vehicle_msgs__msg__HandBrakeReport *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_auto_vehicle_msgs__msg__HandBrakeReport__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_auto_vehicle_msgs__msg__HandBrakeReport__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  for (size_t i = 0; i < input->size; ++i) {
    if (!autoware_auto_vehicle_msgs__msg__HandBrakeReport__copy(
