  ...
  autoware_auto_vehicle_msgs__msg__GearReport__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_auto_vehicle_msgs__msg__GearReport * data =
      (autoware_auto_vehicle_msgs__msg__GearReport *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_auto_vehicle_msgs__msg__GearReport__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_auto_vehicle_msgs__msg__GearReport__fini(&data[i]);
        }
        free(data);
        return false;
      }
